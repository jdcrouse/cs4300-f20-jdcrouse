const webglUtils = {
  ...
  updateLookAtTranslation: (target, event, index) => {
    target[index] = event.target.value;
    render();
  },
  addShape: (shapes, newShape, type) => {
  ...
  doMouseDown: (event, shapes) => {
    ...
    const shapeType = document.querySelector("input[name='shape']:checked")
      .value;
    const shape = {
      translation,
      rotation,
    ...
    webglUtils.addShape(shapes, shape, shapeType);
  },
  renderCube: (cube) => {
    const geometry = [
      0,
